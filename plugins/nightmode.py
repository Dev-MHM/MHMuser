...
from pyMHMuser.dB.night_db import *
...
@MHMup_cmd(pattern="addnm( (.*)|$)")
async def add_grp(e):
    pat = e.pattern_match.group(1).strip()
    if pat:
        try:
            add_night((await MHMup_bot.get_entity(pat)).id)
...
@MHMup_cmd(pattern="remnm( (.*)|$)")
async def rem_grp(e):
    pat = e.pattern_match.group(1).strip()
    if pat:
        try:
            rem_night((await MHMup_bot.get_entity(pat)).id)
...
@MHMup_cmd(pattern="listnm$")
async def rem_grp(e):
    chats = night_grps()
    name = "NightMode Groups Are-:\n\n"
    for x in chats:
        try:
            ok = await MHMup_bot.get_entity(x)
...
async def open_grp():
    ...
    for chat in chats:
        try:
            ...
            await MHMup_bot.send_message(chat, "**NightMode Off**\n\nGroup Opened 🥳.")
