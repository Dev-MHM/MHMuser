...
from pyMHMuser._misc._assistant import callback, in_pattern
from pyMHMuser.dB._core import HELP, LIST
from pyMHMuser.functions.helper import gen_chlog, time_formatter, updater
from pyMHMuser.functions.misc import split_list
...
SUP_BUTTONS = [
    [
        Button.url("• Repo •", url="https://github.com/Dev-MHM/MHMuser"),
        Button.url("• Support •", url="t.me/MHMuser"),
    ],
]
...
async def inline_handler(event):
    ...
    if INLINE_PIC:
        ...
        result = await event.builder.article(
            title="MHMuser Help Menu", text=text, buttons=_main_help_menu
        )
...
async def _(event):
    ...
    if len(changelog_str) > 1024:
        ...
        with open("MHMup_updates.txt", "w+") as file:
            file.write(tl_chnglog)
        await event.edit(
            get_string("upd_5"),
            file="MHMup_updates.txt",
            buttons=[
                [Button.inline("• Uᴘᴅᴀᴛᴇ Nᴏᴡ •", data="updatenow")],
                [Button.inline("« Bᴀᴄᴋ", data="ownr")],
            ],
        )
        remove("MHMup_updates.txt")
    else:
        await event.edit(
            changelog_str,
            buttons=[
...
async def _(e):
    button = [
        ...
        [
            Button.switch_inline(
                "Piston Eval",
                query="run javascript console.log('Hello MHMuser')",
                same_peer=True,
            ),
            Button.switch_inline(
                "OʀᴀɴɢᴇFᴏx🦊",
        ...
        [
            Button.switch_inline(
                "Tᴡɪᴛᴛᴇʀ Usᴇʀ", query="twitter MHMuser", same_peer=True
            ),
            Button.switch_inline(
...
async def ibuild(e):
    ...
    if pic:
        try:
            ...
            LOGS.exception(er)
    result = [
        await builder.article("MHMuser Op", text=txt, link_preview=False, buttons=btn)
    ]
