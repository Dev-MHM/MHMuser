...
try:
    ...
    WebShot = None
from pyMHMuser.functions.tools import metadata
...
from . import inline_mention, is_url_ok, mediainfo, MHMup_cmd


@MHMup_cmd(pattern="tr", manager=True)
async def _(event):
...
@MHMup_cmd(pattern="bots( (.*)|$)", groups_only=True, manager=True)
async def _(ult):
...
async def _(e):
    ...
    if "audio" in mediainfo(reply.media):
        ...
        try:
            ...
            bbbb = "resources/extras/MHMuser.jpg"
        im = cv2.imread(bbbb)
        dsize = (512, 512)
...
async def _(e):
    ...
    try:
        ...
        with io.BytesIO(str.encode(text)) as out_file:
            ...
            await e.reply(
                f"`{e.text}`", file=out_file, thumb="resources/extras/MHMuser.jpg"
            )
        await e.delete()
