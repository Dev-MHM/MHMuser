...
from . import get_string, inline_mention, udB, MHMup_bot, MHMup_cmd


@MHMup_cmd(pattern="addsudo( (.*)|$)", fullsudo=True)
async def _(ult):
...
@MHMup_cmd(pattern="delsudo( (.*)|$)", fullsudo=True)
async def _(ult):
    inputs = ult.pattern_match.group(1).strip()
    if ult.reply_to_msg_id:
        replied_to = await ult.get_reply_message()
...
async def _(ult):
    ...
    if not m:
        m = "[False](https://telegra.ph/MHMuser-04-06)"
