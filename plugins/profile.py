...
from . import eod, eor, get_string, mediainfo, MHMup_cmd
...
@MHMup_cmd(pattern="setbio( (.*)|$)", fullsudo=True)
async def _(ult):
    ok = await ult.eor("...")
...
@MHMup_cmd(pattern="setname ?((.|//)*)", fullsudo=True)
async def _(ult):
    ok = await ult.eor("...")
...
@MHMup_cmd(pattern="setpic$", fullsudo=True)
async def _(ult):
    if not ult.is_reply:
...
@MHMup_cmd(pattern="delpfp( (.*)|$)", fullsudo=True)
