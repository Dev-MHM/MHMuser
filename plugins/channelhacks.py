...
from pyMHMuser.dB.ch_db import (
    add_destination,
    add_source_channel,
    get_destinations,
    get_no_destinations,
...
from . import LOGS, asst, eor, events, get_string, udB, MHMup_bot, MHMup_cmd
...
async def source(e):
    ...
    if not is_source_channel_added(y):
        ...
        MHMup_bot.add_handler(autopost_func, events.NewMessage())
    elif is_source_channel_added(y):
        await e.eor(get_string("cha_3"))


@MHMup_cmd(pattern="dsource( (.*)|$)")
async def dd(event):
...
@MHMup_cmd(pattern="listdest")
async def list_all(event):
    MHMup_bot = event.client
    x = await event.eor(get_string("com_1"))
    ...
    for channel in channels:
        name = ""
        try:
            name = get_display_name(await MHMup_bot.get_entity(int(channel)))
        except BaseException:
            name = ""
        msg += f"\n=> **{name}** [`{channel}`]"
