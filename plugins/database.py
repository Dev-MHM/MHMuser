...
from . import Redis, eor, get_string, udB, MHMup_cmd


@MHMup_cmd(pattern="setdb( (.*)|$)", fullsudo=True)
async def _(ult):
...
@MHMup_cmd(pattern="deldb( (.*)|$)", fullsudo=True)
async def _(ult):
...
@MHMup_cmd(pattern="rendb( (.*)|$)", fullsudo=True)
async def _(ult):
    match = ult.pattern_match.group(1).strip()
    if not match:
