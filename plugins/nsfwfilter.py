...
from pyMHMuser.dB.nsfw_db import is_nsfw, nsfw_chat, rem_nsfw

from . import HNDLR, async_searcher, eor, events, udB, MHMup_bot, MHMup_cmd


@MHMup_cmd(pattern="addnsfw( (.*)|$)", admins_only=True)
async def addnsfw(e):
    ...
    MHMup_bot.add_handler(nsfw_check, events.NewMessage(incoming=True))
    await e.eor("Added This Chat To Nsfw Filter")


@MHMup_cmd(pattern="remnsfw", admins_only=True)
async def remnsfw(e):
    rem_nsfw(e.chat_id)
        ...
        if nsfw:
            ...
            if NWARN.get(e.sender_id):
                ...
                if count < 3:
                    ...
                    return await MHMup_bot.send_message(
                        chat,
                ...
                if "mute" in action:
                    ...
                    try:
                        await MHMup_bot.kick_participant(chat, e.sender_id)
                        await MHMup_bot.send_message(
                            chat,
                            f"NSFW Warn 3/3 to [{e.sender.first_name}](tg://user?id={e.sender_id})\n\n**Action Taken** : {action}",
                        )
