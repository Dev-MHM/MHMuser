...
@MHMup_cmd(pattern="purge( (.*)|$)", manager=True, require="delete_messages")
...
async def fastpurgerme(purg):
    ...
    async for msg in purg.client.iter_messages(
        ...
        if len(msgs) == 100:
            await MHMup_bot.delete_messages(chat, msgs)
            msgs = []
...
@MHMup_cmd(pattern="pinned", manager=True, groups_only=True)
