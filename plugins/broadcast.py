...
from pyMHMuser.dB.broadcast_db import *
from telethon.utils import get_display_name

from . import HNDLR, LOGS, eor, get_string, udB, MHMup_bot, MHMup_cmd
...
async def forw(event):
    ...
    for channel in channels:
        try:
            await MHMup_bot.forward_messages(channel, previous_message)
            sent_count += 1
            await x.edit(
                f"Sent : {sent_count}\nError : {error_count}\nTotal : {len(channels)}",
            )
...
async def sending(event):
    ...
    if event.reply_to_msg_id:
        ...
        if previous_message:
            ...
            for channel in channels:
                try:
                    await MHMup_bot.send_message(channel, previous_message)
                    sent_count += 1
                    await x.edit(
                        f"Sent : {sent_count}\nError : {error_count}\nTotal : {len(channels)}",
                    )
