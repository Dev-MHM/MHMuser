...
from pyMHMuser.dB.notes_db import add_note, get_notes, list_note, rem_note
...
@MHMup_cmd(pattern="addnote( (.*)|$)", admins_only=True)
async def an(e):
    ...
    MHMup_bot.add_handler(notes, events.NewMessage())


@MHMup_cmd(pattern="remnote( (.*)|$)", admins_only=True)
async def rn(e):
    wrd = (e.pattern_match.group(1).strip()).lower()
    chat = e.chat_id
    if not wrd:
...
@MHMup_cmd(pattern="listnote$", admins_only=True)
async def lsnote(e):
