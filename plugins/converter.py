...
from . import bash, con, downloader, get_paste, get_string, udB, MHMup_cmd, uploader
...
async def _(e):
    ...
    await bash(f"wget {nn} -O resources/extras/MHMuser.jpg")
...
async def uconverter(event):
    ...
    file = await con.convert(b, outname="MHMuser", convert_to=convert)
    if file:
...
async def _(event):
    ...
    await event.reply(file=input_str, thumb="resources/extras/MHMuser.jpg")
    await xx.delete()
    os.remove(input_str)
