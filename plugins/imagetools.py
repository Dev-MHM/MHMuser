...
async def sketch(e):
    ...
    cv2.imwrite("MHMuser.png", pencil_sketch_IMG)
    await e.reply(file="MHMuser.png")
    await xx.delete()
    os.remove(file)
    os.remove("MHMuser.png")


@MHMup_cmd(pattern="color$")
async def _(event):
...
async def ultd(event):
    ...
    file = await con.convert(ultt, convert_to="png", outname="ult")
    ult = cv2.imread(file)
    MHMuser = cv2.cvtColor(ult, cv2.COLOR_BGR2GRAY)
    cv2.imwrite("ult.jpg", MHMuser)
...
async def ultd(event):
    ...
    file = await con.convert(ultt, convert_to="png", outname="ult")
    ult = cv2.imread(file)
    MHMuser = cv2.GaussianBlur(ult, (35, 35), 0)
    cv2.imwrite("ult.jpg", MHMuser)
    await event.client.send_file(
        event.chat_id,
        "ult.jpg",
...
async def ultd(event):
    ...
    MHMuser = cv2.cvtColor(dan, cv2.COLOR_HSV2BGR)
    cv2.imwrite("ult.jpg", MHMuser)
    await event.client.send_file(
        event.chat_id,
        "ult.jpg",
...
async def pixelator(event):
    ...
    await msg.respond("• Pixelated by MHMuser", file="output.jpg")
    await msg.delete()
