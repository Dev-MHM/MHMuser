...
from . import LOGS, get_string, udB, MHMup_bot, MHMup_cmd


@MHMup_cmd(pattern="autopic( (.*)|$)")
async def autopic(e):
...
if search := udB.get_key("AUTOPIC"):
    ...
    async def autopic_func():
        ...
        file = await MHMup_bot.upload_file(img)
        await MHMup_bot(UploadProfilePhotoRequest(file))
        shuffle(ok)
