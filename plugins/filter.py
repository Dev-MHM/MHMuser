...
from pyMHMuser.dB.filter_db import add_filter, get_filter, list_filter, rem_filter
...
async def af(e):
    ...
    MHMup_bot.add_handler(filter_func, events.NewMessage())


@MHMup_cmd(pattern="remfilter( (.*)|$)")
async def rf(e):
    wrd = (e.pattern_match.group(1).strip()).lower()
    chat = e.chat_id
    if not wrd:
