...
from pyMHMuser.dB.blacklist_db import (
    add_blacklist,
    get_blacklist,
    list_blacklist,
    rem_blacklist,
...
from . import events, get_string, udB, MHMup_bot, MHMup_cmd


@MHMup_cmd(pattern="blacklist( (.*)|$)", admins_only=True)
async def af(e):
    ...
    MHMup_bot.add_handler(blacklist, events.NewMessage(incoming=True))
    await e.eor(get_string("blk_2").format(wrd))


@MHMup_cmd(pattern="remblacklist( (.*)|$)", admins_only=True)
async def rf(e):
    wrd = e.pattern_match.group(1).strip()
    chat = e.chat_id
    if not wrd:
...
@MHMup_cmd(pattern="listblacklist$", admins_only=True)
async def lsnote(e):
