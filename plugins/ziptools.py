...
async def do_zip(event):
    ...
    if event.pattern_match.group(1).strip():
        await bash(
            f"zip -r --password {event.pattern_match.group(1).strip()} MHMuser.zip zip/*"
        )
    else:
        await bash("zip -r MHMuser.zip zip/*")
    k = time.time()
    xxx = await uploader("MHMuser.zip", "MHMuser.zip", k, xx, get_string("com_6"))
    await event.client.send_file(
        event.chat_id,
        xxx,
        force_document=True,
