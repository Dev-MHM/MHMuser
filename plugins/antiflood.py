...
from pyMHMuser.dB.antiflood_db import get_flood, get_flood_limit, rem_flood, set_flood
from pyMHMuser.functions.admins import admin_check
from telethon.events import NewMessage as NewMsg

from . import Button, Redis, asst, callback, eod, get_string, MHMup_bot, MHMup_cmd
...
async def unmuting(e):
    ...
    user = int(ino[0])
    chat = int(ino[1])
    user_name = (await MHMup_bot.get_entity(user)).first_name
    chat_title = (await MHMup_bot.get_entity(chat)).title
    await MHMup_bot.edit_permissions(chat, user, send_messages=True)
    await e.edit(
