...
from pyMHMuser.dB import stickers
from pyMHMuser.dB.forcesub_db import get_forcesetting
from pyMHMuser.dB.gban_mute_db import is_gbanned
from pyMHMuser.dB.greetings_db import get_goodbye, get_welcome, must_thank
from pyMHMuser.dB.nsfw_db import is_profan
from pyMHMuser.functions.helper import inline_mention
from pyMHMuser.functions.tools import async_searcher, create_tl_btn, get_chatbot_reply
...
try:
    ...
    detector = None
from . import LOG_CHANNEL, LOGS, asst, get_string, types, udB, MHMup_bot
from ._inline import something


@MHMup_bot.on(events.ChatAction())
...
async def DummyHandler(ult):
    ...
    if (
        ...
        if not user.bot:
            ...
            try:
                await MHMup_bot(GetParticipantRequest(int(joinchat), user.id))
            except UserNotParticipantError:
                await MHMup_bot.edit_permissions(
                    ult.chat_id, user.id, send_messages=False
                )
                res = await MHMup_bot.inline_query(
    ...
    if ult.user_joined or ult.added_by:
        ...
        # gbans and @MHMuserBans checks
        if udB.get_key("MHMup_BANS"):
            try:
                is_banned = await async_searcher(
                    "https://bans.MHMsupport/api/status",
                    json={"userId": user.id},
                    post=True,
                    re_json=True,
                )
                if is_banned["is_banned"]:
                    ...
                    await ult.client.send_message(
                        chat.id,
                        "**@MHMuserBans:** Banned user detected and banned!\n`{}`.\nBan reason: {}".format(
                            str(is_banned),
                            is_banned["reason"],
                        ),
                    )
...
@MHMup_bot.on(events.NewMessage(incoming=True))
...
@MHMup_bot.on(events.Raw(types.UpdateUserName))
