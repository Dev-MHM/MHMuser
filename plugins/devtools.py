...
@MHMup_cmd(pattern="bash", fullsudo=True, only_devs=True)
async def _(event):
    carb = None
    try:
    ...
    if len(OUT) > 4096:
        ...
        with BytesIO(str.encode(ultd)) as out_file:
            ...
            await event.client.send_file(
                ...
                out_file,
                force_document=True,
                thumb="resources/extras/MHMuser.jpg",
                allow_cache=False,
...
@MHMup_cmd(pattern="eval", fullsudo=True, only_devs=True)
async def _(event):
    try:
        cmd = event.text.split(" ", maxsplit=1)[1]
    except IndexError:
    ...
    if any(item in cmd for item in KEEP_SAFE().All) and (
        not (event.out or event.sender_id == MHMup_bot.uid)
    ):
...
@MHMup_cmd(pattern="cpp", only_devs=True)
async def doie(e):
    ...
    open("cpp-MHMuser.cpp", "w").write(match)
    m = await bash("g++ -o CppMHMuser cpp-MHMuser.cpp")
    o_cpp = f"• **Eval-Cpp**\n`{match}`"
    if m[1]:
        o_cpp += f"\n\n**• Error :**\n`{m[1]}`"
        if len(o_cpp) > 3000:
            os.remove("cpp-MHMuser.cpp")
            if os.path.exists("CppMHMuser"):
                os.remove("CppMHMuser")
