...
from pyMHMuser.dB.warn_db import add_warn, reset_warn, warns

from . import eor, get_string, inline_mention, udB, MHMup_cmd
...
async def warn(e):
    ...
    if reply:
        ...
        try:
            user = e.text.split()[1]
            if user.startswith("@"):
                ok = await MHMup_bot.get_entity(user)
                user = ok.id
            else:
                user = int(user)
    ...
    if count + 1 >= number:
        if "ban" in action:
            try:
                await MHMup_bot.edit_permissions(e.chat_id, user, view_messages=False)
            except BaseException:
                return await e.eor("`Something Went Wrong.`", time=5)
        elif "kick" in action:
            try:
                await MHMup_bot.kick_participant(e.chat_id, user)
            except BaseException:
                return await e.eor("`Something Went Wrong.`", time=5)
        elif "mute" in action:
            try:
...
@MHMup_cmd(pattern="setwarn( (.*)|$)", manager=True)
async def warnset(e):
