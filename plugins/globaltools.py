...
from pyMHMuser.dB.gban_mute_db import (
    gban,
    gmute,
    is_gbanned,
    is_gmuted,
...
from pyMHMuser.dB.gcast_blacklist_db import (
    add_gblacklist,
    is_gblacklisted,
    rem_gblacklist,
)
from pyMHMuser.functions.tools import create_tl_btn, format_btn, get_msg_button
...
@MHMup_cmd(pattern="gpromote( (.*)|$)", fullsudo=True)
async def _(e):
...
@MHMup_cmd(pattern="gdemote( (.*)|$)", fullsudo=True)
async def _(e):
    x = e.pattern_match.group(1).strip()
    MHMup_bot = e.client
    if not x:
    ...
    if user:
        ...
        if user.isdigit():
            user = int(user)
        try:
            name = await MHMup_bot.get_entity(user)
        except BaseException:
            return await e.eor(f"`No User Found Regarding {user}`", time=5)
        ...
        rank = "Not AdMin"
        c = 0
        async for x in MHMup_bot.iter_dialogs():
            if (
                "group" in key.lower()
                and x.is_group
                or "group" not in key.lower()
...
@MHMup_cmd(pattern="ungban( (.*)|$)", fullsudo=True)
async def _(e):
...
@MHMup_cmd(pattern="gban( (.*)|$)", fullsudo=True)
async def _(e):
...
@MHMup_cmd(pattern="g(admin|)cast( (.*)|$)", fullsudo=True)
...
@MHMup_cmd(pattern="gucast( (.*)|$)", fullsudo=True)
...
@MHMup_cmd(pattern="gkick( (.*)|$)", fullsudo=True)
async def gkick(e):
...
@MHMup_cmd(pattern="gmute( (.*)|$)", fullsudo=True)
async def _(e):
...
@MHMup_cmd(pattern="ungmute( (.*)|$)", fullsudo=True)
async def _(e):
...
async def list_gengbanned(event):
    ...
    if len(gbanned_users) > 4096:
        ...
        await x.reply(
            file="gbanned.txt",
            message=f"List of users GBanned by {inline_mention(MHMup_bot.me)}",
        )
...
@MHMup_cmd(pattern="gblacklist$")
async def blacklist_(event):
    await gblacker(event, "add")


@MHMup_cmd(pattern="ungblacklist$")
