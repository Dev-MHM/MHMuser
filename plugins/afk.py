...
from pyMHMuser.dB.afk_db import add_afk, del_afk, is_afk
from pyMHMuser.dB.pmpermit_db import is_approved
...
@MHMup_cmd(pattern="afk( (.*)|$)", owner_only=True)
async def set_afk(event):
    ...
    MHMup_bot.add_handler(remove_afk, events.NewMessage(outgoing=True))
    ...
    if text and media:
        if "sticker" in media_type:
            msg1 = await MHMup_bot.send_file(event.chat_id, file=media)
            msg2 = await MHMup_bot.send_message(
                event.chat_id, get_string("afk_5").format(text)
            )
        else:
            msg1 = await MHMup_bot.send_message(
                event.chat_id, get_string("afk_5").format(text), file=media
            )
    elif media:
        if "sticker" in media_type:
            msg1 = await MHMup_bot.send_file(event.chat_id, file=media)
            msg2 = await MHMup_bot.send_message(event.chat_id, get_string("afk_6"))
        else:
            msg1 = await MHMup_bot.send_message(
