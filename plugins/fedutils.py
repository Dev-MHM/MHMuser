...
from . import get_string, udB, MHMup_bot, MHMup_cmd
...
async def _(event):
        ...
        for a in range(3):
            async with event.client.conversation("@MissRose_bot") as bot_conv:
                ...
                if "make a file" in response.text or "Looks like" in response.text:
                    ...
                    await asyncio.sleep(3)
                    if fedfile.media:
                        downloaded_file_name = await MHMup_bot.download_media(
                            fedfile,
                            "fedlist",
                        )
    ...
    try:
        await MHMup_bot.send_message(chat, "/start")
    except BaseException:
    ...
    for fed in fedList:
        if udB.get_key("EXCLUDE_FED") and fed in excludeFed:
            await MHMup_bot.send_message(chat, f"{fed} Excluded.")
            exCount += 1
            continue
...
async def _(event):
    ...
    for fed in fedList:
        if udB.get_key("EXCLUDE_FED") and fed in excludeFed:
            ...
            exCount += 1
            continue
        await MHMup_bot.send_message(chat, f"/joinfed {fed}")
        await asyncio.sleep(3)
        await MHMup_bot.send_message(chat, f"/unfban {FBAN} {REASON}")
        await asyncio.sleep(3)
    try:
        os.remove("fedlist")
    except Exception as e:
    ...
    if sysarg == "":
        ...
        async with event.client.conversation(bot) as conv:
            try:
                ...
                await MHMup_bot.send_read_acknowledge(bot)
