...
from pyMHMuser.dB.forcesub_db import add_forcesub, get_forcesetting, rem_forcesub
...
@MHMup_cmd(pattern="fsub( (.*)|$)", admins_only=True, groups_only=True)
async def addfor(e):
    ...
    MHMup_bot.add_handler(force_sub, events.NewMessage(incoming=True))


@MHMup_cmd(pattern="remfsub$")
async def remor(e):
...
async def fcall(e):
    match = e.pattern_match.group(1).strip()
    spli = match.split("_")
    user = await MHMup_bot.get_entity(int(spli[0]))
    cl = await MHMup_bot.get_entity(int(spli[1]))
    text = f"Hi {inline_mention(user)}, You Need to Join"
    text += f" {cl.title} in order to Chat in this Group."
    if not cl.username:
        el = (await MHMup_bot(ExportChatInviteRequest(cl))).link
    else:
...
async def diesoon(e):
    ...
    try:
        values = await MHMup_bot(GetParticipantRequest(int(spli[1]), int(spli[0])))
...
async def force_sub(ult):
    ...
    if count in range(2, 11):
        return
    try:
        await MHMup_bot.get_permissions(int(joinchat), user.id)
        return
    except UserNotParticipantError:
        pass
    if isinstance(user, Channel):
        try:
            ...
            LOGS.exception(er)
    try:
        await MHMup_bot.edit_permissions(ult.chat_id, user.id, send_messages=False)
    except ChatAdminRequiredError:
        return
    except Exception as e:
        await ult.delete()
        LOGS.info(e)
    res = await MHMup_bot.inline_query(asst.me.username, f"fsub {user.id}_{joinchat}")
