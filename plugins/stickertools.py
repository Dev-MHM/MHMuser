...
async def pack_kangish(_):
    ...
    if len(_.text) > 9:
        ...
        _packname = f"MHMuser Kang Pack By {_.sender_id}"
...
async def hehe(args):
    ...
    if message.photo:
        photo = io.BytesIO()
        photo = await MHMup_bot.download_media(message.photo, photo)
    elif message.file and "image" in message.file.mime_type.split("/"):
        photo = io.BytesIO()
        await MHMup_bot.download_file(message.media.document, photo)
        if (
    ...
    if photo:
        ...
        if not emoji:
            emoji = "🏵"
        if len(splat) == 3:
            pack = splat[2]  # User sent MHMup_both
            emoji = splat[1]
        elif len(splat) == 2:
        ...
        if (
            ...
            async with MHMup_bot.conversation("@Stickers") as conv:
                try:
                    await conv.send_message("/addsticker")
                except YouBlockedUserError:
                    LOGS.info("Unblocking @Stickers for kang...")
                    await MHMup_bot(functions.contacts.UnblockRequest("stickers"))
                    await conv.send_message("/addsticker")
                ...
                await conv.get_response()
                await MHMup_bot.send_read_acknowledge(conv.chat_id)
        else:
...
async def ultdestroy(event):
    ...
    await event.client.download_media(ult, "MHMuser.tgs")
    xx = await event.eor(get_string("com_1"))
    await bash("lottie_convert.py MHMuser.tgs json.json")
    ...
    file = await con.animated_sticker("json.json", "MHMuser.tgs")
    if file:
        await event.client.send_file(
            event.chat_id,
            file="MHMuser.tgs",
...
async def ultiny(event):
    ...
    im1 = Image.open("resources/extras/MHMup_blank.png")
    if ik.endswith(".tgs"):
