...
from pyMHMuser.dB.filestore_db import del_stored, get_stored_msg, list_all_stored_msgs
from pyMHMuser.functions.tools import get_file_link

from . import HNDLR, asst, get_string, in_pattern, udB, MHMup_bot, MHMup_cmd


@MHMup_cmd(pattern="store$")
...
async def _(event):
    ...
    del_stored(match[1])
    await MHMup_bot.delete_messages(udB.get_key("LOG_CHANNEL"), int(msg_id))
    await event.eor("__Deleted__")


@MHMup_cmd("liststored$")
