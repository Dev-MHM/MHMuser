...
from pyMHMuser._misc._assistant import asst_cmd
...
@MHMup_cmd(pattern="kickme$", fullsudo=True)
async def leave(ult):
    me = asst.me if ult.client._bot else MHMup_bot.me
...
@MHMup_cmd(pattern="paste( (.*)|$)", manager=True, allow_all=True)
async def _(event):
    try:
...
async def telegraphcmd(event):
    match = event.pattern_match.group(1).strip() or "MHMuser"
    reply = await event.get_reply_message()
    if not reply:
...
@MHMup_cmd(pattern="suggest( (.*)|$)", manager=True)
