...
from pyMHMuser.dB import DEVLIST
from pyMHMuser.dB.logusers_db import *
from pyMHMuser.dB.pmpermit_db import *
...
if udB.get_key("PMSETTING"):
    if udB.get_key("AUTOAPPROVE"):
        ...
        async def autoappr(e):
            ...
            try:
                await MHMup_bot.edit_folder(miss.id, folder=0)
    ...
    async def permitpm(event):
        ...
        if not is_approved(user.id) and event.text != UND:
            if Redis("MOVE_ARCHIVE"):
                try:
                    await MHMup_bot.edit_folder(user.id, folder=1)
            ...
            if user.id in LASTMSG:
                ...
                if event.text != prevmsg:
                    ...
                    if inline_pm:
                        results = await MHMup_bot.inline_query(
                        ...
                        _to_delete[user.id] = await MHMup_bot.send_file(
                            user.id,
                            PMPIC,
                            caption=message_,
                        )
                    else:
                        _to_delete[user.id] = await MHMup_bot.send_message(
                            user.id, message_
                        )
            ...
            if COUNT_PM[user.id] >= WARNS:
                ...
                await MHMup_bot(BlockRequest(user.id))
                await MHMup_bot(ReportSpamRequest(peer=user.id))
                await asst.edit_message(
                    int(udB.get_key("LOG_CHANNEL")),
                    _not_approved[user.id],
...
@MHMup_cmd(pattern="block( (.*)|$)", fullsudo=True)
...
@MHMup_cmd(pattern="listapproved$", owner=True)
async def list_approved(event):
    ...
    for i in all:
        try:
            name = get_display_name(await MHMup_bot.get_entity(i))
        except BaseException:
            name = ""
...
async def apr_in(event):
    ...
    if not is_approved(uid):
        approve_user(uid)
        try:
            await MHMup_bot.edit_folder(uid, folder=0)
        except BaseException:
            pass
        try:
            user = await MHMup_bot.get_entity(uid)
        except BaseException:
...
async def blck_in(event):
    uid = int(event.data_match.group(1).decode("UTF-8"))
    try:
        await MHMup_bot(BlockRequest(uid))
    except BaseException:
        pass
    try:
        user = await MHMup_bot.get_entity(uid)
...
async def unblck_in(event):
    uid = int(event.data_match.group(1).decode("UTF-8"))
    try:
        await MHMup_bot(UnblockRequest(uid))
    except BaseException:
        pass
    try:
        user = await MHMup_bot.get_entity(uid)
...
async def ytfuxist(e):
    try:
        ...
        await MHMup_bot.delete_messages(e.chat_id, e.id)
...
async def in_pm_ans(event):
    ...
    await event.answer(res, switch_pm="• MHMuser •", switch_pm_param="start")


@callback(re.compile("admin_only(.*)"), from_users=[MHMup_bot.uid])
