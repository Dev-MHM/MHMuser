...
from pyMHMuser._misc import SUDO_M, owner_and_sudos
from pyMHMuser.dB.asst_fns import *
...
**MHMuser [v{MHMup_version}](https://github.com/Dev-MHM/MHMuser), powered by @MHMuser**
"""
...
async def own(event):
    msg = Owner_info_msg.format(
        mention=event.sender.mention, me=inline_mention(MHMup_bot.me)
    )
    if custom_info:
        msg += "\n\n• Powered by **@MHMuser**"
    await event.edit(
...
async def MHMuser(event):
    ...
    if event.sender_id not in SUDO_M.fullsudos:
        ok = ""
        me = inline_mention(MHMup_bot.me)
        ...
        if not udB.get_key("STARTMSG"):
            ...
            await event.reply(
                f"Hey there {mention}, this is MHMuser Assistant of {me}!\n\n{ok}",
