...
try:
    from pyMHMuser.functions.gDrive import GDriveManager
except ImportError:
    GDriveManager = None
from pyMHMuser.functions.helper import fast_download, progress, uploader
...
from pyMHMuser.startup.loader import Loader
...
_buttons = {
    "otvars": {
        "text": "Other Variables to set for @MHMuser:",
        "buttons": [
            [
    ...
    "oofdm": {
        "text": "About [Dual Mode](https://t.me/MHMuser/18)",
        "buttons": [
            [
...
async def update(eve):
    ...
    if heroku_api:
        ...
        execl(sys.executable, sys.executable, "-m", "pyMHMuser")
    ...
    if not match:
        try:
            ...
            return await okk.edit(
                f"**• MHMuser Userbot •**{cli}", file=img, buttons=button
            )
        except Exception as er:
            LOGS.exception(er)
...
async def _(e):
    ...
    async with asst.conversation(e.sender_id) as conv:
        ...
        if GDrive._create_token_file(code=code.text):
            await conv.send_message(
                "`Success!\nYou are all set to use Google Drive with MHMuser Userbot.`",
...
async def _(e):
    if not e.is_private:
        return
    await e.edit(
        "Click Authorise and send the code.\n\nYou can use your own CLIENT ID and SECRET by [this](https://t.me/MHMuser/37)",
        buttons=[
            [
