...
def telethon_session():
    ...
    # logging in
    try:
        with TelegramClient(StringSession(), API_ID, API_HASH) as MHMuser:
            print("Generating a string session for •MHMuser•")
            try:
                ult = MHMuser.send_message(
                    "me",
                    f"**MHMuser** `SESSION`:\n\n`{MHMuser.session.save()}`\n\n**Do not share this anywhere!**",
                )
                print(
        ...
        print(er)
        print("If you think It as a Bug, Report to @MHMuser.\n\n")
